...
class VitUpscaler(nn.Module):
    def __init__(self, base_model = "fastvit_ma36", factor = 2, input_shape = (3, 112, 112), out_shape = (3, 224, 224)):
        ...
        self.entry1 = self.encoder.patch_embed[0]
        self.entry2 = self.encoder.patch_embed[1]
        self.entry3 = self.encoder.patch_embed[2]
    ...
    def forward(self, x):
        print("Input: ", x.shape)
        xp = self.prep(x)
        print("After prep: ", xp.shape)

        xe1 = self.entry1(x)
        print("After entry1: ", xe1.shape)
        xe2 = self.entry2(xe1)
        print("After entry2: ", xe2.shape)
        xe3 = self.entry3(xe2)
        print("After entry3: ", xe3.shape)
